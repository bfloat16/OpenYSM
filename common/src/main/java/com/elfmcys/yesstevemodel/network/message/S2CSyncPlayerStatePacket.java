    ...
    public static S2CSyncPlayerStatePacket decode(FriendlyByteBuf buffer) {
        ...
        if ((flags & 4) != 0) {
            ...
            if (effectCount == 0) {
                ...
                message.effectAmplifiers = Object2ByteMaps.singleton(BuiltInRegistries.MOB_EFFECT.asHolderIdMap().byId(buffer.readVarInt()), buffer.readByte());
            } else {
                Holder[] effects = new Holder[effectCount];
                byte[] amplifiers = new byte[effectCount];
                for (int i = 0; i < effectCount; i++) {
                    effects[i] = BuiltInRegistries.MOB_EFFECT.asHolderIdMap().byId(buffer.readVarInt());
